...
def remove_user_from_telegram_bot(user_id):
    """Удаляет пользователя из Telegram бота (вызывается из веб API)"""
    try:
        user_id_int = int(user_id)
        
        # Проверяем что пользователь существует
        if user_id_int not in BotConfig.AUTHORIZED_USERS:
            return False, "Пользователь не найден в списке авторизованных"
        
        # Проверяем что не пытаемся удалить админа
        if user_id_int == BotConfig.ADMIN_ID:
            return False, "Нельзя удалить администратора"
        
        # Удаляем пользователя
        BotConfig.AUTHORIZED_USERS.remove(user_id_int)
        ...
        try:
            ...
            async def notify_user():
                try:
                    ...
                    logger.error(f"Ошибка отправки уведомления пользователю {user_id_int}: {e}")
            
            loop.run_until_complete(notify_user())
            loop.close()
            
        except Exception as e:
            logger.error(f"Ошибка уведомления пользователя {user_id_int}: {e}")
        
        logger.info(f"✅ Пользователь {user_id_int} удален из Telegram бота")
        return True, f"Пользователь {user_id_int} успешно удален"
