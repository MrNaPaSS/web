...
def generate_signal_analysis(prompt):
    """Генерация анализа сигнала на основе промпта"""
    # Простой анализ на основе ключевых слов
    if 'успешную' in prompt.lower() or 'успешно' in prompt.lower():
        return """✅ АНАЛИЗ УСПЕШНОЙ СДЕЛКИ:
1️⃣ Трейдер правильно выбрал направление и следовал стратегии.
2️⃣ Ключевые факторы успеха: точный анализ рынка, терпение и верная оценка условий.
3️⃣ Рекомендации: найти оптимальную точку входа, продолжать применять стратегию, масштабировать успех и развивать аналитические навыки.
💪 Отлично! Продолжай в том же духе! Зарабатывай еще больше!"""
    
    elif 'убыточную' in prompt.lower() or 'убыток' in prompt.lower():
        return """🔴 АНАЛИЗ УБЫТОЧНОЙ СДЕЛКИ:
1️⃣ Трейдер допустил ошибки при входе в опцион и не дождался лучшего момента.
2️⃣ Психологические ошибки: эмоциональные решения, FOMO, жадность или паника.
3️⃣ Рекомендации: найти лучшую точку входа, изменить подход, избежать ошибок в будущем.
💪 Не сдавайся! Каждая сделка - это опыт! Продолжай торговать и зарабатывай!"""
    
    else:
        return """📊 АНАЛИЗ СИГНАЛА:
1️⃣ Проверьте технические индикаторы и рыночные условия.
2️⃣ Убедитесь в правильности выбора направления и времени входа.
3️⃣ Следуйте стратегии управления рисками.
💪 Успешной торговли!"""
